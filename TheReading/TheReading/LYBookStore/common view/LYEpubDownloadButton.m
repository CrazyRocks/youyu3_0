...
- (void)changeBackground
{
    ...
    UIImageView *viewInfoReal;
    for (id viewInfo in [self subviews]) {
        if ([viewInfo isKindOfClass:[UIImageView class]]) {
            viewInfoReal = (UIImageView *)viewInfo;
    ...
    if (currentStatus == lyHighlighted) {
        ...
        color = @"#5FB7E7";
        borderColor = color;
        viewInfoReal.image = [UIImage imageNamed:@"book_button_read"];
    }
    else if (currentStatus == lyDownloading) {
        color = @"#BDBDBD";
        borderColor = color;
        viewInfoReal.image = [UIImage imageNamed:@"empty"];
    }
    else if (currentStatus == lyContinueBorrow || currentStatus == lyFree) {
        color = @"#ffffff";
        textColor = @"#005870";
        borderColor = @"#005870";
    ...
    tileLable.textColor = [OWColor colorWithHexString:textColor];
    [self.layer setBorderColor:[OWColor colorWithHexString:borderColor].CGColor];
}
...
- (void)setBookInfo:(LYBookItemData *)bookItem
{
    ...
    //NSLog(@"\r\n status:%ld, guid:%@", downloadStatus, bookInfo.bGUID);
    if (downloadStatus == 1) {
        currentStatus = lyDownloaded;
        tileLable.text = @"阅 读";
    }
