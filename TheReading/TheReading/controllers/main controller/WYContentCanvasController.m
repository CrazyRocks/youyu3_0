...
- (id)init
{
    ...
    if (self) {
        ...
        g_accountBtn = self.accountButton;
    }
    return self;
}
...
- (void)openMenu:(LYMenuData *)menu
{
    ...
    if ([menu.menuType isEqualToString:@"article"]) { //资讯
        [self intoArticleList:menu];
    }
    else if ([moduleCode isEqualToString:@"magazine"]) { //杂志
        [self intoMagazineStore];
    }
    else if ([moduleCode isEqualToString:@"book"]) { //图书
        [self intoBookStore];
    }
...
- (void)intoOfflineByIndex: (NSInteger)index
{
    ...
    offlineController.pageIndex = index;
    [self animateToShowController:offlineController];
}
...
- (void)intoAttentionView
{
    ...
    [self animateToShowController:focusedMagCollectionController];
}
...
{
    [OWPublicLib setCurrentTitle:title];
...
- (void)animateToShowController:(OWViewController *)controller
{
    ...
    if (controller == magazinesStandController ||
        ...
        self.accountButton.hidden = YES;
        //[leftButton setBackgroundImage:[UIImage imageNamed:@"back_button_navigation"] forState:UIControlStateNormal];
