...
- (id)init
{
    if (self = [super initWithNibName:@"WYMainMenuController" bundle:nil]) {
        //del by yzj, 查看有关通知注册，函数为空，未实现其它功能，该处通知可不注册。
...
- (void)viewDidLoad
{
    ...
    bgColor = [OWColor colorWithHexString:@"#2B3036"];
    if (isPad) {
        bgView.hidden = YES;
    ...
    _tableView.separatorColor = [OWColor colorWithHexString:@"#22262B"];
    [self hideEmptyRows:_tableView];
}

- (void)hideEmptyRows: (UITableView *)tableview {
...
{
    //NSLog(@"FUN:%s;FILE:%s;LINE:%d", __func__, __FILE__, __LINE__);
