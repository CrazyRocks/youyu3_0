...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    WYContentCanvasController *parentCtl = (WYContentCanvasController *)[self parentViewController].parentViewController;
    if (currentController.class == NULL || currentController.class == [LYMagazineStoreController class]) {

        parentCtl.accountButton.hidden = NO;
        
    } else {
        parentCtl.accountButton.hidden = YES;
    }
    //NSLog(@"\r\n currentController:%@", currentController.class);
}
...
- (void)tabBar:(UITabBar *)theTabBar didSelectItem:(UITabBarItem *)item
{
    ...
    if (tempController && tempController != currentController) {
        ...
        WYContentCanvasController *parentCtl = (WYContentCanvasController *)[self parentViewController].parentViewController;
        if (tempController.class == NULL || tempController.class == [LYMagazineStoreController class]) {
            //NSLog(@"\r\nno class:%@", [tempController class]);
            [self performSelector:@selector(setAccountHiddenState:) withObject:nil afterDelay:0.5f];
            
        } else {
            //NSLog(@"\r\nyes class:%@", [tempController class]);
            parentCtl.accountButton.hidden = YES;
        }
...
- (void)setAccountHiddenState: (BOOL)bState {
    WYContentCanvasController *parentCtl = (WYContentCanvasController *)[self parentViewController].parentViewController;
    parentCtl.accountButton.hidden = NO;
}
