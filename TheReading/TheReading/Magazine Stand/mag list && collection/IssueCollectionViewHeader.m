...
- (void)awakeFromNib
{
    ...
    flowView = [[LYFlowNavControl alloc]initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 200)];
    flowView.backgroundColor = [UIColor redColor];
    [self addSubview:flowView];
    
}

- (void)getListForMagazine {
    flowView.dataList = self.dataArray;
}
...
- (void)setImgList {
    if (self.dataArray) {
        ...
        for (int i = 0; i < [self.dataArray count]; i++) {
            LYMagazineTableCellData *dicData = [self.dataArray objectAtIndex:i];
            NSString *url = [IssueCollectionViewHeader getImgURL:dicData];
