...
static CGFloat fontSizeCurrent = appFontsize_normal;
...
-(void)viewDidLoad
{
    ...
    fontSizeCurrent = [defaults floatForKey:APP_FONTSIZE];
    if (fontSizeCurrent < appFontsize_min || fontSizeCurrent > appFontsize_max) {
        fontSizeCurrent = appFontsize_normal;

    }
    [defaults setFloat:fontSizeCurrent forKey:APP_FONTSIZE];
    [defaults synchronize];
    //[fontSizeButton setSelected: (fontSizeCurrent > appFontsize_normal)];
...
-(IBAction)fontsizeButton_add:(id)sender {
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    [fontSizeReduce setSelected:NO];
    [fontSizeAdd setSelected:YES];
    fontSizeCurrent++;
    if (fontSizeCurrent > appFontsize_max) {
        fontSizeCurrent = appFontsize_max;
    }
    [defaults setFloat:fontSizeCurrent forKey:APP_FONTSIZE];
    [defaults synchronize];
    
    [[NSNotificationCenter defaultCenter] postNotificationName:APP_FONTSIZE_CHANGED object:nil];
    //NSLog(@"\r\n fontsizeadd:%f", fontSizeCurrent);
    /*if ([defaults floatForKey:APP_FONTSIZE]==appFontsize_normal) {
        [defaults setFloat:appFontsize_large forKey:APP_FONTSIZE];
        [fontSizeButton setSelected:YES];
    }
...
-(IBAction)fontsizeButton_reduce:(id)sender {
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    [fontSizeAdd setSelected:NO];
    [fontSizeReduce setSelected:YES];
    fontSizeCurrent--;
    if (fontSizeCurrent < appFontsize_min) {
        fontSizeCurrent = appFontsize_min;
    }
    [defaults setFloat:fontSizeCurrent forKey:APP_FONTSIZE];
    [defaults synchronize];
    
    [[NSNotificationCenter defaultCenter] postNotificationName:APP_FONTSIZE_CHANGED object:nil];
    //NSLog(@"\r\n fontsizereduce:%f", fontSizeCurrent);
}
