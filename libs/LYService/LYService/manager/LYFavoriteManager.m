...
//            favorite.addDate = [formatter dateFromString:[[item objectForKey:@"CreateDate"] isEqualToString:@""] ? @"0" :[item objectForKey:@"CreateDate"]];
            ...
            //NSLog(@"\r\n title:%@,date:%@", item[@"Title"], item[@"UpdateDate"]);
            if (item[@"UpdateDate"]) {
                NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
                ...
                favorite.publishDate = [formatter dateFromString:[item objectForKey:@"UpdateDate"]];
                favorite.publishDateSection = [formatter2 stringFromDate:favorite.publishDate];
            }
...
- (LYArticleTableCellData *)parseFavorite:(LYFavorite *)favorite
{
    ...
    art.publishDate = favorite.publishDate;
    art.publishDateSection = favorite.publishDateSection;
    return art;
}
