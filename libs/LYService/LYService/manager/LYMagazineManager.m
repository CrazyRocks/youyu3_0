                       ...
                       fault:(GLHttpRequstFault)fCallBack
{
    ...
    completionBlock = ^(NSDictionary *result) {
        ...
        //NSLog(@"\r\n name:%@,dateformatted:%@", mag.magName, magInfo.FormattedIssue);
...
-(LYMagCatelogueTableCellData *)parseArticleListItem:(NSDictionary *)dic column:(NSString *)column
{
    ...
    //NSLog(@"\r\n magname:%@,year:%@-%@", article.magName, [CommonNetworkingManager sharedInstance].currentMagazine.year, [CommonNetworkingManager sharedInstance].currentMagazine.issue);
...
+ (void)attentionMagazine:(LYMagazineTableCellData *)cellData bySender:(UIButton *)sender {
    //[sender setEnabled:NO];
    [self focusMagazine:cellData
             completion:^{
                 
                 [sender setBackgroundImage:[UIImage imageNamed:@"magazine_attention_true"] forState:UIControlStateNormal];
                 [[OWSlideMessageViewController sharedInstance] showMessage:@"关注成功" autoClose:YES];
             } fault:^{
                 //[sender setEnabled:YES];
                 [sender setBackgroundImage:[UIImage imageNamed:@"magazine_attention_false"] forState:UIControlStateNormal];
                 [[OWSlideMessageViewController sharedInstance] showMessage:@"关注失败" autoClose:YES];
             }];
}
